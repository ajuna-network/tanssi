...
use {
    frame_support::{pallet_prelude::*, PalletId},
    frame_system::pallet_prelude::*,
};

use account::*;
pub use {config::*, traits::*};
...
pub mod pallet {
    use {
        super::*,
        frame_support::traits::{Currency, ExistenceRequirement},
        sp_arithmetic::traits::AtLeast16BitUnsigned,
        sp_runtime::{
    ...
    impl<T: Config<I>, I: 'static> Pallet<T, I> {
        ...
        fn ensure_valid_tournament(
            ...
            let current_block = <frame_system::Pallet<T>>::block_number();
                ...
                current_block < config.start,
                Error::<T, I>::InvalidTournamentConfig
            );
            ensure!(
                config.start < config.active_end,
                Error::<T, I>::InvalidTournamentConfig
            );
            ensure!(
                config.active_end < config.claim_end,
                Error::<T, I>::InvalidTournamentConfig
            );
            ...
            let reward_table_total_dist = config
                .reward_distribution
                .iter()
                .fold(0_u16, |a, b| a + (*b as u16));
        ...
        fn try_update_rank_table(
            ...
            if index < tournament_config.max_players as usize {
                ...
                table
                    ...
                    .map(|_| RankingResult::Ranked {
                        rank: index.saturated_into(),
                    })
