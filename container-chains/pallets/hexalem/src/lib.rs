...
pub mod pallet {
        ...
        pub fn get_stats(&self) -> BoardStats {
            ...
            for tile in self.hex_grid.clone() {
                ...
                stats.set_levels(
                    ...
                    tile.get_level(),
                    stats
                        .get_levels(tile_type, tile.get_level() as usize)
                        .saturating_add(1),
                );
                ...
                stats.set_patterns(
                    ...
                    tile.get_pattern(),
                    stats
                        .get_patterns(tile_type, tile.get_pattern())
                        .saturating_add(1),
                );
            }
    ...
    pub enum Event<T: Config> {
        ...
        NewTurn {
            game_id: GameId,
            next_player: T::AccountId,
        },

        // Game has finished
        GameFinished {
            game_id: GameId, /* , winner: T::AccountId */
        },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_game(
            ...
            for player in &players {
                    ...
                    !HexBoardStorage::<T>::contains_key(player),
        ...
        pub fn upgrade(origin: OriginFor<T>, place_index: u8) -> DispatchResult {
            ...
            Self::deposit_event(Event::TileUpgraded {
                game_id,
                player: who,
                place_index,
            });
        ...
        pub fn finish_turn(origin: OriginFor<T>) -> DispatchResult {
            ...
            if Self::is_game_won(&hex_board) {
                game.state = GameState::Finished {
                    winner: Some(game.get_player_turn()),
                };
            }
        ...
        pub fn force_finish_turn(origin: OriginFor<T>, game_id: GameId) -> DispatchResult {
            ...
            let mut game = match GameStorage::<T>::get(&game_id) {
                ...
                game.borrow_players().contains(&who),
                Error::<T>::PlayerNotInGame
            );
...
impl<T: Config> Pallet<T> {
    ...
    fn buy_from_selection(
        ...
        // Select the offer
            ...
            selection.len() > index_to_buy,
            Error::<T>::BuyIndexOutOfBounds
        );
        ...
        let all_offers = T::TileCosts::get();
            ...
            all_offers.len() > selected_offer_index as usize,
            Error::<T>::BuyIndexOutOfBounds
        );
        ...
        for neighbour in neighbours {
            match neighbour {
                Some(value) => {
                    ...
                    n.push(Some((
                        neighbour_index,
                        hex_board.hex_grid[neighbour_index as usize],
                    )));
                }
                None => n.push(None),
            }
