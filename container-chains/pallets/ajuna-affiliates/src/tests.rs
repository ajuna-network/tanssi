...
mod affiliate_to {
    ...
    fn affiliate_to_should_work() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            assert_eq!(
                Affiliatees::<Test, Instance1>::get(ALICE),
                Some(bounded_vec![BOB])
            );
        });
    }
...
mod multi_instance_tests {
    ...
    fn affiliates_in_one_instance_dont_affect_other_instance() {
        ExtBuilder::default()
            ...
            .build()
            .execute_with(|| {
                let state_alpha = AffiliatorState {
                    status: AffiliatableStatus::Affiliatable(0),
                    affiliates: 0,
                };
                ...
                let state_beta = AffiliatorState {
                    status: AffiliatableStatus::Affiliatable(0),
                    affiliates: 0,
                };
                ...
                assert_eq!(
                    Affiliatees::<Test, Instance2>::get(CHARLIE),
    ...
    fn rule_in_one_instance_doesnt_affect_other_instance() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                assert_eq!(
                    AffiliateRules::<Test, Instance1>::get(rule_id),
                    Some(rule.clone())
                );
                // But not on Instance2
                assert_eq!(AffiliateRules::<Test, Instance2>::get(rule_id), None);

                <AffiliatesBeta as RuleMutator<MockRuleId, MockRuntimeRule>>::clear_rule_for(
                    rule_id,
                );
