...
use {
    ...
    sp_runtime::{bounded_vec, testing::H256, BoundedVec},
};
...
mod store_as_nft {
    use {super::*, sp_runtime::traits::Get};

    #[test]
    fn can_store_item_successfully() {
        ExtBuilder::default()
            .balances(&[
                (ALICE, CollectionDeposit::get() + 999),
                (BOB, ItemDeposit::get() + 999),
            ])
            .build()
            .execute_with(|| {
    ...
    fn can_store_item_and_transfer() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                assert_ok!(Nft::do_transfer(collection_id, item_id, ALICE, |_, _| {
                    Ok(())
                }));
