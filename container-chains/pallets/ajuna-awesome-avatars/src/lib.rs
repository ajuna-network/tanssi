...
use {
    crate::{types::*, weights::WeightInfo},
    frame_support::{
        pallet_prelude::*,
        traits::{Currency, ExistenceRequirement::AllowDeath, Randomness, WithdrawReasons},
        PalletId,
    },
    frame_system::{ensure_root, ensure_signed, pallet_prelude::*},
    pallet_ajuna_affiliates::traits::{
        AffiliateInspector, AffiliateMutator, RuleInspector, RuleMutator,
    },
    pallet_ajuna_nft_transfer::traits::NftHandler,
...
pub mod pallet {
    use {
        super::*,
        pallet_ajuna_affiliates::traits::{AffiliateId, RuleExecutor},
        pallet_ajuna_tournament::{Percentage, TournamentId},
    ...
    pub enum Event<T: Config> {
        ...
        /// Avatar forged.
        AvatarsForged {
            avatar_ids: Vec<(AvatarIdOf<T>, UpgradedComponents)>,
        },
        /// Avatar transferred.
        AvatarTransferred {
            from: T::AccountId,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn transfer_avatar(
            ...
            let from = match Self::ensure_organizer(origin.clone()) {
                ...
                Self::ensure_for_trade(&avatar_id).is_err(),
            ...
            let avatar = Self::ensure_ownership(&from, &avatar_id)?;
                ...
                PlayerSeasonConfigs::<T>::get(&from, avatar.season_id)
                    .locks
                    .avatar_transfer,
            ...
            Self::deposit_event(Event::AvatarTransferred {
                from,
                to,
                avatar_id,
            });
        ...
        pub fn set_price(
            ...
            let seller = ensure_signed(origin)?;
                ...
                GlobalConfigs::<T>::get().trade.open,
                Error::<T>::TradeClosed
            );
            let avatar = Self::ensure_ownership(&seller, &avatar_id)?;
            ensure!(
                PlayerSeasonConfigs::<T>::get(&seller, avatar.season_id)
                    .locks
                    .set_price,
                Error::<T>::FeatureLocked
            );
        ...
        pub fn buy(origin: OriginFor<T>, avatar_id: AvatarIdOf<T>) -> DispatchResult {
            ...
            Self::deposit_event(Event::AvatarTraded {
                avatar_id,
                from: seller,
                to: buyer,
                price,
        ...
        pub fn upgrade_storage(
            ...
            let storage_tier =
                ...
                storage_tier != StorageTier::Max,
                Error::<T>::MaxStorageTierReached
            );
        ...
        pub fn set_treasurer(
            ...
            Self::deposit_event(Event::TreasurerSet {
                season_id,
                treasurer,
            });
            Ok(())
        ...
        pub fn claim_treasury(origin: OriginFor<T>, season_id: SeasonId) -> DispatchResult {
            ...
            Self::deposit_event(Event::TreasuryClaimed {
                season_id,
                treasurer,
                amount,
            });
        ...
        pub fn lock_avatar(origin: OriginFor<T>, avatar_id: AvatarIdOf<T>) -> DispatchResult {
            ...
            let avatar = Self::ensure_ownership(&player, &avatar_id)?;
                ...
                Error::<T>::NftTransferClosed
            );
            Self::ensure_unlocked(&avatar_id)?;
            ensure!(
                Preparation::<T>::contains_key(avatar_id),
                Error::<T>::NotPrepared
            );
        ...
        pub fn unlock_avatar(origin: OriginFor<T>, avatar_id: AvatarIdOf<T>) -> DispatchResult {
            ...
            let avatar = Self::ensure_ownership(&Self::technical_account_id(), &avatar_id)?;
                ...
                LockedAvatars::<T>::contains_key(avatar_id),
                Error::<T>::AvatarUnlocked
            );
            ...
            match target {
                UnlockTarget::OneselfFree => {
                    ...
                    if let Some(UnlockConfigs {
                        affiliate_unlock: Some(unlock_vec),
                        ..
        ...
        pub fn enable_set_avatar_price(
            ...
            match target {
                UnlockTarget::OneselfFree => {
                    if let Some(UnlockConfigs {
                        set_price_unlock: Some(unlock_vec),
                        ..
                    }) = SeasonUnlocks::<T>::get(season_id)
                    {
                        let player_stats = SeasonStats::<T>::get(season_id, &account);
        ...
        pub fn set_unlock_config(
            ...
            Self::deposit_event(Event::UpdatedUnlockConfigs {
                season_id,
                unlock_configs,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(crate) fn ensure_organizer(
            ...
            let existing_organizer = Organizer::<T>::get().ok_or(Error::<T>::OrganizerNotSet)?;
                ...
                maybe_organizer == existing_organizer,
        ...
        pub(crate) fn do_mint(player: &T::AccountId, mint_option: &MintOption) -> DispatchResult {
            ...
            SeasonStats::<T>::mutate(season_id, player, |info| {
                info.minted
                    .saturating_accrue(generated_avatar_ids.len() as Stat);
            ...
            Self::deposit_event(Event::AvatarsMinted {
                avatar_ids: generated_avatar_ids,
            });
            Ok(())
        }
        ...
        pub(crate) fn do_forge(
            ...
            let input_sacrifices = sacrifice_ids
                .into_iter()
                .zip(sacrifices)
                .collect::<Vec<ForgeItem<T>>>();
        ...
        pub(crate) fn ensure_for_mint(
            ...
            if !last_block.is_zero() {
                    ...
                    current_block >= last_block + mint.cooldown,
                    Error::<T>::MintCooldown
                );
            }
            ...
            let is_free_mint = mint_option.payment == MintPayment::Free;
                ...
                active || early && (is_whitelisted || is_free_mint),
            ...
            let max_count = player_season_config.storage_tier as u16;
                ...
                current_count + mint_count <= max_count,
                Error::<T>::MaxOwnershipReached
            );
            Ok(())
        }
        ...
        > {
            ...
            let (season_id, season) = Self::season_with_id_for(&leader)?;
                ...
                sacrifice_count >= season.min_sacrifices,
                Error::<T>::TooFewSacrifices
            );
            ensure!(
                sacrifice_count <= season.max_sacrifices,
                Error::<T>::TooManySacrifices
            );
            ensure!(
                !sacrifice_ids.contains(leader_id),
                Error::<T>::LeaderSacrificed
            );
            ensure!(
                sacrifice_ids
                    .iter()
                    .all(|id| Self::ensure_for_trade(id).is_err()),
                Error::<T>::AvatarInTrade
            );
            ensure!(
                Self::ensure_for_trade(leader_id).is_err(),
                Error::<T>::AvatarInTrade
            );
        ...
        fn ensure_tradable(avatar: &AvatarOf<T>) -> DispatchResult {
            let trade_filters = SeasonTradeFilters::<T>::get(avatar.season_id)
                ...
                trade_filters.is_tradable(avatar),
                Error::<T>::AvatarCannotBeTraded
            );
            Ok(())
        }
