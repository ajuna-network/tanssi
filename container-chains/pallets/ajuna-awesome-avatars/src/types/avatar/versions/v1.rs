use {
    ...
    sp_std::{collections::btree_set::BTreeSet, marker::PhantomData, vec::Vec},
};
...
impl<T: Config> MinterV1<T> {
    ...
    fn random_component(
        ...
        let random_tier = {
            ...
            let probs = if batched_mint {
                &season.batch_mint_probs
            } else {
                &season.single_mint_probs
            };
...
pub(crate) struct ForgerV1<T: Config>(pub PhantomData<T>);

impl<T: Config> Forger<T> for ForgerV1<T> {
    fn forge(
        player: &T::AccountId,
        _season_id: SeasonId,
        season: &SeasonOf<T>,
...
impl<T: Config> ForgerV1<T> {
    ...
    fn compare(
        ...
        let (matching_indexes, matches, mirrors) = target
            .dna
            .clone()
            .into_iter()
            .zip(other.dna.clone())
...
#[cfg(test)]
mod test {
    use {super::*, crate::mock::*, frame_support::assert_ok};
    ...
    fn forge_probability_works() {
        ...
        let now = 1;
        assert_eq!(
            ForgerV1::<Test>::forge_probability(&avatar, &season, &now, 1),
            25
        );
        assert_eq!(
            ForgerV1::<Test>::forge_probability(&avatar, &season, &now, 2),
            50
        );
        assert_eq!(
            ForgerV1::<Test>::forge_probability(&avatar, &season, &now, 3),
            75
        );
        assert_eq!(
            ForgerV1::<Test>::forge_probability(&avatar, &season, &now, 4),
            100
        );
    ...
    fn forge_multiplier_works() {
        ...
        for (range, dna, expected_period, expected_multiplier) in [
            ...
            for now in range {
                ...
                assert_eq!(
                    ForgerV1::<Test>::forge_multiplier(&avatar, &season, &now),
                    expected_multiplier
                );
            }
        }
    ...
    fn compare_works() {
        ...
        let leader = Avatar::default().dna(&[
            0x21, 0x05, 0x23, 0x24, 0x20, 0x22, 0x25, 0x23, 0x05, 0x04, 0x02,
        ]);
        let other = Avatar::default().dna(&[
            0x04, 0x00, 0x00, 0x04, 0x02, 0x04, 0x02, 0x00, 0x05, 0x05, 0x04,
        ]);

        assert_eq!(
            ForgerV1::<Test>::compare(
                &leader,
                &other,
                &[1, 8, 9, 10],
                season.max_variations,
        ...
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                let upgradable_indexes =
                    ForgerV1::<Test>::upgradable_indexes_for_target(&original_leader).unwrap();
                for (sacrifice, result) in original_sacrifices.iter().zip([
                    (true, BTreeSet::from([0, 2, 3])),
                    (true, BTreeSet::from([0, 2, 3, 4])),
                ]) {
                    assert_eq!(
                        ForgerV1::<Test>::compare(
                            &original_leader,
                ...
                assert_eq!(
                    original_leader.dna.to_vec()[0] >> 4,
                    RarityTier::Common as u8
                );
                assert_eq!(
                    original_leader.dna.to_vec()[1] >> 4,
                    RarityTier::Common as u8
                );
                assert_eq!(
                    forged_leader.dna.to_vec()[0] >> 4,
                ...
                System::assert_last_event(mock::RuntimeEvent::AAvatars(
                    crate::Event::AvatarsForged {
                        avatar_ids: vec![(leader_id, 1)],
                    },
                ));

                // variations remain the same
                assert_eq!(
                    original_leader.dna[0..=1]
                        .iter()
                        .map(|x| x & 0b0000_1111)
                        .collect::<Vec<_>>(),
                    forged_leader.dna[0..=1]
                        .iter()
                        .map(|x| x & 0b0000_1111)
                        .collect::<Vec<_>>(),
                );
