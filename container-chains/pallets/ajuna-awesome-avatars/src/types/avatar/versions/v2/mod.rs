...
#[cfg(test)]
pub use test_utils::*;
pub use {avatar_utils::*, combinator::*, constants::*, mutator::*, slot_roller::*, types::*};
...
impl ForgeType {
    pub(crate) fn is_restricted(&self) -> bool {
        matches!(
            self,
            ForgeType::Tinker | ForgeType::Build | ForgeType::Mate | ForgeType::Glimmer
        )
    }
}
...
impl<T: Config> Forger<T> for ForgerV2<T> {
    fn forge(
        ...
        let sacrifices = input_sacrifices
            ...
            .collect::<Vec<_>>();
        let wrapped_sacrifices = sacrifices
            .iter()
            .map(|(_, avatar)| avatar)
            .collect::<Vec<_>>();
            ...
            ItemType::Pet => match leader.get_item_sub_type::<PetItemType>() {
                ...
                PetItemType::PetPart => {
                        ...
                        })
                    {
                        ...
                        && sacrifices
                            .iter()
                            .all(|sacrifice| sacrifice.has_type(ItemType::Material))
                    {
                        ForgeType::Tinker
                    } else {
                        ForgeType::None
                ...
                PetItemType::Egg => {
                    ...
                    if leader_rarity <= RarityTier::Epic
                        && sacrifices
                            .iter()
                            .all(|sacrifice| sacrifice.same_full_type(leader))
                    {
                        ForgeType::Breed
                    } else {
                        ForgeType::None
...
#[cfg(test)]
mod test {
    use {super::*, crate::mock::*, sp_std::collections::btree_map::BTreeMap};
    ...
    fn test_determine_forge_types_breed() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let sacrifices =
                [
                    &create_random_egg(None, &ALICE, &RarityTier::Common, 0b0001_0010, 10, [2; 11])
                        .1,
                ];
            assert_eq!(
                ForgerV2::<Test>::determine_forge_type(&leader, &sacrifices),
            ...
            let (_, leader_err) = create_random_egg(
                None,
                &ALICE,
    ...
    fn test_determine_forge_types_mate() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let sacrifices = [&create_random_pet(
                &ALICE,
                &PetType::CrazyDude,
                0b0001_0001,
                [0; 16],
