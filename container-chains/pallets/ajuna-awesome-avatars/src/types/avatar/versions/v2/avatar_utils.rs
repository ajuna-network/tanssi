use {
    super::{constants::*, types::*, ByteType},
    crate::{
    ...
    BlockNumber: sp_runtime::traits::BlockNumber,
{
    ...
    pub fn same_item_sub_type(&self, other: &WrappedAvatar<BlockNumber>) -> bool {
        self.get_item_sub_type::<u8>()
            .cmp(&other.get_item_sub_type::<u8>())
            .is_eq()
    }
    ...
    pub fn same_class_type_1(&self, other: &WrappedAvatar<BlockNumber>) -> bool {
        self.get_class_type_1::<u8>()
            .cmp(&other.get_class_type_1::<u8>())
            .is_eq()
    }
    ...
    pub fn same_class_type_2(&self, other: &WrappedAvatar<BlockNumber>) -> bool {
        self.get_class_type_2::<u8>()
            .cmp(&other.get_class_type_2::<u8>())
            .is_eq()
    }
    ...
    pub fn same_custom_type_1(&self, other: &WrappedAvatar<BlockNumber>) -> bool {
        self.get_custom_type_1::<u8>()
            .cmp(&other.get_custom_type_1::<u8>())
            .is_eq()
    }
    ...
    pub fn same_custom_type_2(&self, other: &WrappedAvatar<BlockNumber>) -> bool {
        self.get_custom_type_2::<u8>()
            .cmp(&other.get_custom_type_2::<u8>())
            .is_eq()
    }
    ...
    BlockNumber: sp_runtime::traits::BlockNumber,
{
    ...
    pub fn into_generic_pet_part(self, slot_types: &[SlotType], quantity: u8) -> Self {
        ...
        let spec_bytes = {
            ...
            for slot_index in slot_types
                .iter()
                .map(|slot_type| slot_type.as_byte() as usize)
            {
    ...
    BlockNumber: sp_runtime::traits::BlockNumber,
{
    ...
    pub fn match_progress(
        ...
        for i in 0..array_1.len() {
            ...
            if have_same_rarity
                && !is_maxed
                && (rarity_1 < rarity_level || variation_2 == 0x0B || byte_match)
            {
        ...
        T: ByteConvertible + Ranged,
    {
        ...
        enum_list.iter().fold(0_u32, |acc, entry| {
            acc | (1 << (entry.as_byte().saturating_sub(range_mod)))
        })
    }
...
mod test {
    ...
    fn test_enum_to_bits_consistency_1() {
        ...
        assert_eq!(
            DnaUtils::<BlockNumberFor<Test>>::enums_to_bits(&pattern),
            expected
        );
    }
    ...
    fn test_enum_order_to_bits_consistency() {
        ...
        assert_eq!(
            DnaUtils::<BlockNumberFor<Test>>::enums_order_to_bits(&pattern),
            expected
        );
    }
    ...
    fn test_match_progress_array_consistency() {
        ...
        let arr_1 = [
            0x00, 0x11, 0x02, 0x13, 0x04, 0x15, 0x04, 0x13, 0x02, 0x11, 0x00,
        ];
        let arr_2 = [
            0x01, 0x01, 0x12, 0x13, 0x04, 0x04, 0x13, 0x12, 0x01, 0x01, 0x15,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
    ...
    fn test_match_progress_array_consistency_multiple() {
        ...
        let arr_2 = [
            0x12, 0x13, 0x14, 0x13, 0x14, 0x11, 0x22, 0x10, 0x14, 0x22, 0x11,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_2 = [
            0x10, 0x10, 0x10, 0x14, 0x14, 0x13, 0x13, 0x12, 0x15, 0x14, 0x14,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_2 = [
            0x15, 0x10, 0x14, 0x13, 0x13, 0x11, 0x10, 0x14, 0x12, 0x20, 0x11,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_2 = [
            0x11, 0x11, 0x11, 0x10, 0x15, 0x12, 0x11, 0x11, 0x13, 0x12, 0x14,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
    ...
    fn test_match_progress_consistency_on_level() {
        ...
        let arr_1 = [
            0x42, 0x40, 0x40, 0x44, 0x43, 0x42, 0x41, 0x44, 0x44, 0x42, 0x45,
        ];
        let arr_2 = [
            0x41, 0x51, 0x52, 0x53, 0x44, 0x52, 0x45, 0x41, 0x40, 0x41, 0x43,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_2 = [
            0x52, 0x41, 0x43, 0x41, 0x53, 0x45, 0x43, 0x44, 0x52, 0x43, 0x43,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_1 = [
            0x42, 0x40, 0x40, 0x44, 0x43, 0x42, 0x41, 0x54, 0x44, 0x42, 0x53,
        ];
        let arr_2 = [
            0x52, 0x40, 0x43, 0x41, 0x53, 0x45, 0x41, 0x44, 0x52, 0x43, 0x43,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
        ...
        let arr_1 = [
            0x45, 0x45, 0x45, 0x45, 0x45, 0x45, 0x45, 0x30, 0x30, 0x30, 0x30,
        ];
        let arr_2 = [
            0x45, 0x45, 0x45, 0x45, 0x45, 0x35, 0x45, 0x31, 0x30, 0x45, 0x45,
        ];
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
    ...
    fn test_match_progress_consistency_hex() {
        ...
        let arr_1: [u8; 11] = hex::decode("3130353330333132323234")
            .expect("Decode")
            .try_into()
            .unwrap();
        let arr_2: [u8; 11] = hex::decode("2121353424332322222222")
            .expect("Decode")
            .try_into()
            .unwrap();
        let (mirrors, matches) = DnaUtils::<BlockNumberFor<Test>>::match_progress(arr_1, arr_2, 0);
    ...
    fn test_indexes_of_max() {
        ExtBuilder::default().build().execute_with(|| {
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::indexes_of_max(&[0, 2, 1, 1]),
                vec![1]
            );
            assert_eq!(
    ...
    fn test_current_period() {
        ExtBuilder::default().build().execute_with(|| {
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 0),
                0
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 9),
                0
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 10),
                1
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 19),
                1
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 130),
                13
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 139),
                13
            );
            assert_eq!(
                DnaUtils::<BlockNumberFor<Test>>::current_period::<Test>(10, 14, 140),
                0
            );
        });
    }
