...
impl<T: Config> SlotRoller<T> {
    ...
    pub(crate) fn roll_number<const HS: usize>(hash_provider: &mut HashProvider<T, HS>) -> u16 {
        let first_number = (hash_provider.next() as u16) << 8;
        let second_number = hash_provider.next() as u16;
...
mod test {
    use {
        super::{super::types::*, *},
        crate::{mock::*, types::ByteConvertible, Pallet},
    };
    ...
    fn statistics_verification_test() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let packs: [ProbabilitySlots<MaterialItemType, 2>; 3] = [
                [
                    (MaterialItemType::Polymers, 500),
                    (MaterialItemType::Electronics, 500),
                ],
                [
                    (MaterialItemType::Polymers, 300),
                    (MaterialItemType::Electronics, 700),
                ],
                [
                    (MaterialItemType::Polymers, 900),
                    (MaterialItemType::Electronics, 100),
                ],
            ];
