...
impl<T: Config> AvatarCombinator<T> {
    pub(super) fn mate_avatars(
        ...
        if input_sacrifices.len() != 1
            ...
            || input_sacrifices
                .iter()
                .any(|(_, sac)| sac.get_rarity() == RarityTier::Mythical)
        {
            return Ok((
        ...
        let (leader_output, other_output, any_survived) = if partner.get_souls() > soul_points {
            ...
            (
                ...
                vec![
                    ForgeOutput::Consumed(partner_id),
                    ForgeOutput::Minted(generated_dust),
                ],
                false,
            )
        };
        ...
        Ok((
            leader_output,
            other_output.into_iter().chain(additional_output).collect(),
        ))
    }
}
...
mod test {
    ...
    fn test_mate_extra_sacrifices() {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let extra_partner = create_random_pet(
                &ALICE,
                &PetType::CrazyDude,
                0b0101_0011,
                [0; 16],
