...
use {
    ...
    sp_runtime::traits::{AtLeast32Bit, UniqueSaturatedInto, Zero},
    sp_std::{
        borrow::ToOwned,
        ops::{Deref, DerefMut},
    },
...
impl<BlockNumber: AtLeast32Bit, Balance> Season<BlockNumber, Balance> {
    ...
    pub(crate) fn current_period(&self, now: &BlockNumber) -> u16 {
        let cycles = now
            .checked_rem(&self.full_cycle())
            .unwrap_or_else(Zero::zero);
        let current_period = if cycles.is_zero() {
            Zero::zero()
        } else {
            cycles / self.per_period.clone()
        };
    ...
    fn full_cycle(&self) -> BlockNumber {
        self.per_period
            .clone()
            .saturating_mul(self.periods.unique_saturated_into())
    }
    ...
    fn validate_max_variations<T: Config>(&self) -> DispatchResult {
            ...
            self.max_variations <= 0b0000_1111,
            Error::<T>::MaxVariationsTooHigh
        );
        Ok(())
    }
    ...
    fn validate_max_components<T: Config>(&self) -> DispatchResult {
            ...
            self.max_components
                .checked_mul(2)
                .ok_or(Error::<T>::MaxComponentsTooHigh)? as usize
...
mod test {
    ...
    fn validate_works() {
        ...
        for (mut season, error) in [
            // max_variations
            (
                season.clone().max_variations(0),
                Error::<Test>::MaxVariationsTooLow,
            ),
            (
                season.clone().max_variations(16),
                Error::<Test>::MaxVariationsTooHigh,
            ),
            // max_components
            (
                season.clone().max_components(0),
                Error::<Test>::MaxComponentsTooLow,
            ),
            (
                season.clone().max_components(17),
            ...
            (
                season
                    .clone()
                    .tiers(&[RarityTier::Common, RarityTier::Common]),
            ...
            (
                season.clone().batch_mint_probs(&[1, 100]),
                Error::<Test>::IncorrectRarityPercentages,
            ),
            (
            ...
            (
                season.clone().batch_mint_probs(&[1, 2, 97]),
                Error::<Test>::TooManyRarityPercentages,
            ),
            (
            ...
            (
                season.clone().batch_mint_probs(&[u8::MAX, 1]),
                Error::<Test>::BatchMintProbsOverflow,
            ),
            (
                season.clone().base_prob(101),
                Error::<Test>::BaseProbTooHigh,
            ),
            // periods
            (
                season.clone().per_period(2).periods(u16::MAX),
                Error::<Test>::PeriodConfigOverflow,
            ),
            (
                season.clone().periods(123).max_variations(7),
                Error::<Test>::PeriodsIndivisible,
            ),
        ] {
    ...
    fn validate_schedule_works() {
        for (schedule, error) in [
            // block_numbers
            (
                SeasonSchedule::default().early_start(10).start(0),
                Error::<Test>::EarlyStartTooLate,
            ),
            (
                SeasonSchedule::default().start(10).end(0),
